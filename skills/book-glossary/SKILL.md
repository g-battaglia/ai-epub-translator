...
description: >-
  ...
  attested rendering, and pin them in the book's glossary.toml. Use when a term
...
File: the book's `glossary.toml` (`ai-epub-translator path <slug>` prints the folder),
per book, optional (absent = no effect at all).
...
```bash
b=$(ai-epub-translator path <slug>)
# does the term really occur in the original? how often?
grep -oiE "\bexotéri\w*" $b/.work/original/**/*.html | wc -l

# how does the translation render it? (the decisive datum)
grep -ohiE "\b(esoteri|essoteri)\w*" $b/.work/target/**/*.html | sort | uniq -c

# read the context: what does it mean there?
grep -ohiE '.{60}\bexotérisme.{60}' $b/.work/original/**/*.html | sed 's/<[^>]*>//g' | head -3
```
...
```toml
# <library>/moby-dick/glossary.toml
[terms]
