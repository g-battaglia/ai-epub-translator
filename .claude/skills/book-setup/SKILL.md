...
Nothing here lives inside the harness. `main.py` translates books; deciding a book's
terminology is your job, and it is done with the commands below plus a browser.

## What you are producing

Two files under `books/<slug>/`:
...
python3 - <<'EOF'
import glob, re
for f in sorted(glob.glob('books/<slug>/.work/.work/original/**/*.*htm*', recursive=True))[:12]:
...
import sys, glob, re
sys.path.insert(0, '.')
from traduttore import units as U
from traduttore.config import merged_config

SLUG, TERM = '<slug>', '<the suspect word>'
cfg = merged_config(f'books/{SLUG}')
block = None
for f in sorted(glob.glob(f'books/{SLUG}/.work/original/**/*.*htm*', recursive=True)):
...
1. **Substring collision on the source side.** The check fires whenever the source
   ...
   ```bash
   grep -ohiE "\w*<term>\w*" books/<slug>/.work/.work/original/**/*.*html | sort | uniq -c | sort -rn | head
   ```
...
`--add` writes the terms; open `books/<slug>/glossary.toml` and add the `[notes]`
...
Once some chapters exist, `main.py glossary <slug>` lists each term with ✓ or the count
...
for t in sorted(glob.glob(f'books/{SLUG}/target/**/*.*htm*', recursive=True)):
...
- **`books/` is gitignored** — it holds copyrighted material. Never commit book
  content.
